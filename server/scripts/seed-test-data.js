// Comprehensive seed script for testing all modules
...
async function main() {
  ...
  await prisma.reminderSchedule.deleteMany({ where: { user: { email: { contains: '@testlord.com' } } } });
  await prisma.payment.deleteMany({ where: { tenant: { email: { contains: '@test' } } } });
  await prisma.application.deleteMany({ where: { email: { contains: '@test' } } });
  await prisma.lease.deleteMany({ where: { tenant: { email: { contains: '@test' } } } });
  await prisma.maintenanceTicket.deleteMany({ where: { tenant: { email: { contains: '@test' } } } });
  await prisma.property.deleteMany({ where: { landlord: { email: { contains: '@testlord.com' } } } });
  ...
  // Create test lease (for occupied property)
  ...
  const payment1 = await prisma.payment.create({
    data: {
      ...
      type: 'RENT',
      provider: 'stripe',
      providerIntentId: 'pi_test_paid_12345',
      leaseId: lease1.id,
      tenantId: tenant2.id
    }
  });
  ...
  const payment2 = await prisma.payment.create({
    data: {
      ...
      type: 'RENT',
      provider: 'stripe',
      providerIntentId: 'pi_test_pending_67890',
      leaseId: lease1.id,
      tenantId: tenant2.id
    }
  });
  ...
  // Create test maintenance tickets
  ...
  console.log('✅ Created test maintenance tickets');
  ...
  console.log('\nTest Users Created:');
  console.log('👔 Landlord: landlord@testlord.com / testpass123');
  console.log('🏠 Tenant 1: tenant1@test.com / testpass123');
  console.log('🏠 Tenant 2: tenant2@test.com / testpass123');
  console.log('\nTest Data Summary:');
  ...
  console.log('⏰ Reminders: 2 (1 due soon, 1 future)');
  ...
  console.log(`Reminder 1 ID: ${reminder1.id} (due soon)`);
}
