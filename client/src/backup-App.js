...
useEffect(() => {
  ...
  try {
    // Use the properties endpoint since we know it's working
...
<button
  onClick={() => setCurrentView('tenant-profile')}
  style={styles.navButton(currentView === 'tenant-profile')}
>
  <span>🎯</span>
...
{currentView === 'rental-application' && <RentalApplicationForm />}
{currentView === 'tenant-profile' && <TenantProfile />}
          </>
        )}
      </main>
